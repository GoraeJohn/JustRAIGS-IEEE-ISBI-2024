...
MODEL_PATH = os.path.join(RAW_PATH, "convnext_small_5fold_pretrained_v_flip_mirror_disc_copy_flip")
...
for i in range(0, CFG.fold_num):
    ...
    for epoch in range(1, CFG.max_epoch + 1):
        ...
        hamming_loss = metric_hamming(label_feature, predict_sigmoid, agree_feature, threshold_list)    
        mean_auc = np.mean(roc_auc_list)
        
        log.info(f"[epoch {epoch}] train loss: {train_loss: .6f}, val loss: {val_loss: .6f}, hamming loss: {hamming_loss: .6f}, mean_auc: {mean_auc: .6f}")
        each_auc_str = "Each auc = "
        for current_auc in roc_auc_list:
            each_auc_str += f'{current_auc: .6f}, '
        log.info(each_auc_str)
        
        if hamming_loss < best_hamming:
            best_hamming = hamming_loss
            best_hamming_threshold = threshold_list            
            model.save_model(os.path.join(MODEL_PATH, f'model_hamming_fold_{i}.pth'))
...
np.save(os.path.join(MODEL_PATH, 'best_hamming_threshold.npy'), best_hamming_threshold)
