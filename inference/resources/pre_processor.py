...
class Processor:
    def __init__(self, enable_geometry, target_size, enable_graphy=True):
    ...
    def __process_graphic(self, input_image):             
        ...
        #for channel in range(0, 3):
            #norm_image[:,:,channel] = (resized_image[:,:,channel] - MEAN[channel]) / STD[channel]
        norm_image = input_image / IMAGE_MAX        
        result_image = norm_image.transpose((2, 0, 1))
        result_image = np.expand_dims(result_image, axis=0)
        
        return result_image
    
    def process(self, input_image):
        result_image = input_image #BGR
        if self.__enable_geometry:
            geo_image = self.__process_geometry(input_image, self.__target_size)
        if self.__enable_graphy:            
            geo_image_swap = cv.cvtColor(geo_image, cv.COLOR_BGR2RGB)
            result_image = self.__process_graphic(geo_image_swap) #RGB
